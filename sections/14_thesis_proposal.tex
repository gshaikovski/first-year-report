...
In the few works that do investigate optimal path decoding in search tasks, they find that transformers struggle to search on graphs, either implicating teacher-forcing of critical path tokens that incentivise the model to cheat~\citep{bachmannPitfallsNexttokenPrediction2025} or showing model depth as a bottleneck to learning larger graphs~\citep{saparovTransformersStruggleLearn2025}.
Hence, conclusions from this line of work on the capabilities of transformers for solving search tasks are incomplete, as they fail to consider intermediate computation such as SA reasoning which can help solve these tasks.
...
In the context of search problems, an SO model decodes correct solution paths directly after the problem formulation.
...
\begin{enumerate}
    ...
    \item Can other types of intermediate reasoning, which do not require search traces, achieve performance of SA reasoning?
...
The previous section quantifies the efficiency and performance advantages of SA reasoning over SO reasoning on specific search problems in the search-efficient regime, and suggests the problem structure and its representation alone determine the problem's learning regime.
Next, I propose to expand on this result and (1) demonstrate that the regimes are indeed determined by the problem structure, (2) isolate the factors in its structure and representation that cause a specific regime, and (3) uncover the mechanisms by which they influence the learning process.
...
Searching on an in-context graph makes the model learn an exponential path-merging algorithm~\citep{saparovTransformersStruggleLearn2025}.
...
The current literature and our own experiments suggest potential factors that determine the learning regime: the state graph being in context, the locality of the transition function, or the existence of a cheap heuristic.
...
The goal is to identify the factors that determine the learning regime by performing controlled ablations on the problem structure and observing its effects.
...
\textbf{Can other types of intermediate reasoning, which do not require search traces, achieve performance of SA reasoning?}
...
This raises a question: is SA reasoning required to solve all search problems?
...
Applying these approaches directly to search problems has not yielded useful results yet.
Preliminary experiments with pause and filler tokens showed they are trivially learnable by the model, thereby collapsing the learning task into SO reasoning mode.
...
These preliminary results, paired with our conclusions in the first study, suggest that investigating alternative traces and their performance on search problems requires observing their effect in both search-optional and search-efficient regimes.
...
Although my attempts to find alternative intermediate traces that generalise to search-efficient tasks as SA traces have so far failed, the performance of unrelated traces on modular Countdown provides a possible direction for next steps.
...
Search algorithms that SA models learn to imitate are size-agnostic by construction.
...
At the same time, the performance of neural networks that learn from data is known to be sensitive to how close a given test-time task is to the training distribution~\citep{geirhosShortcutLearning2020}.
Therefore, to close the gap in the analysis of SA reasoning capabilities, the final question of this project asks how well SA reasoning generalises to out-of-distribution (OOD) examples, whether it is better at it than SO reasoning or other forms of intermediate traces, and how it can be improved.

Length generalisation, or size extrapolation --- the ability to solve problems with size that exceeds any of the problems in a training set --- is an important type of OOD generalisation for algorithmic reasoners in general, and SA reasoning models in particular~\citep{anilExploringLengthGeneralization,velickovicCLRSAlgorithmicReasoning2022}.
It is also fundamentally an exact test of OOD generalisation: the target function is defined at every size, so extrapolation identifies which of the many solutions consistent with the training data was actually learned; thus, it is the standard operational test for learning algorithmic reasoning~\citep{zhouWhatAlgorithmsCan2023,anilExploringLengthGeneralization}.
Length generalisation is evidence that a model learned internal representations and token-level outputs that are consistent with a neural implementation of a search algorithm and not some other procedure.
This also matters for interpretability and faithfulness of intermediate traces w.r.t.\ the optimal path solution.
...
Outside search, length generalisation in autoregressive transformers has been studied for arithmetic, logic, and state tracking tasks.
\citet{anilExploringLengthGeneralization} showed that naive training does not extrapolate regardless of model scale, but using prompting and intermediate scratchpad templates with step-by-step solutions improves accuracy.
...
Theoretical studies proposed conditions for length generalisation on algorithmic tasks: transformer can length generalise on a task which can be solved by a short RASP program for any input size~\citep{zhouWhatAlgorithmsCan2023}, or if algorithm execution can follow an ``inductive'' computation flow, i.e., every next step only depends on the current state~\citep{houUniversalLengthGeneralization2024,abbeHowFarCan2024}, with recent theory proving that transformers with chain-of-thought can achieve length generalisation for state tracking \citep{huangTransformersProvablyLearn2025}.
...
A naive maze navigation setup may not be appropriate for testing length generalisation at all, since training on smaller grids ignores embeddings of larger coordinates, leaving them untrained; such a setup cannot length generalise to larger grids by construction.
\citet{caiExtrapolationAssociationLength2025} circumvents this problem by fixing the grid size constant and instead sampling mazes with a smaller number of nodes within the grid.
This results in smaller mazes but full coverage of all coordinates.
They find that training with depth-first search~(DFS)~\citep{russel2010} traces does not generalise to larger mazes; however, adding an auxiliary task that \textit{does} cover larger maze sizes lifts DFS trace performance on the same sizes.
This is the only study that directly addresses length extrapolation in SA reasoning and the only study that shows a positive result.
...
The mechanism of length generalisation here is not algorithmic size extrapolation but the transfer of maze-traversal capability to larger maze sizes from SO to SA reasoning, which is nevertheless an interesting observation and can provide a basis for future work.
...
For example, taking the best SA model trained on modular Countdown with length 3, which was evaluated in the first study, and testing it on length-4 and length-5 puzzles results in a severe drop in performance.
This setup therefore serves as a control baseline.
To improve length generalisation, I turn to current transformer theory reviewed at the beginning of this section, which suggests that, with the right problem structure and trace format, length generalisation can be achieved for an arbitrary task.
The gap, therefore, is that the conditions on structure and trace that the theory predicts to enable length generalisation have not been instantiated on search problems, neither with SA nor SO reasoning.
Applying this theory to search problems will also stress-test its predictions and claims of universality, as search tasks are arguably more challenging to learn than arithmetic or state tracking tasks investigated in the theory literature.
The most obvious first step is to attempt to convert search traces into ``inductive'' format such that the next step only depends on the previous one, which can help~\citep{abbeHowFarCan2024,houUniversalLengthGeneralization2024}; the trace format in prior work and our first study requires a transformer to track intermediate search states across the entire trace.
...
Finally, comparing these interventions in search-efficient and search-optional regimes will be necessary to establish whether they generalise search itself or if they merely add useful shortcuts.
Addressing length generalisation in transformers for search problems promises to lead the field to review current language model training practices that excel at ID capabilities but fail to generalise OOD.
