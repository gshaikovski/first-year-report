...
It is also fundamentally an exact test of OOD generalisation: the target function is defined at every size, so extrapolation identifies which of the many solutions consistent with the training data was actually learned; thus, it is the standard operational test for learning algorithmic reasoning~\citep{zhouWhatAlgorithmsCan2023,anilExploringLength2022}.
...
The most obvious first step is to attempt to convert search traces into ``inductive'' format such that the next step only depends on the previous one, which can help~\citep{abbeHowFarCan2024,houTuringPrograms2024}; the trace format in prior work and our first study requires a transformer to track intermediate search states across the entire trace.
