...
It may especially benefit many complex tasks that involve a form of search, for example, planning, mathematical problems, theorems proving, and, more speculatively, scientific discovery~\citep{newell1972human,langleyScientificDiscovery1987}.
...
To improve length generalisation, I turn to current transformer theory reviewed at the beginning of this section which suggests that, with the right problem structure and trace format, length generalisation can be achieved on an arbitrary task.
...
The most obvious first step is to attempt to convert search traces into ``inductive'' format such that the next step only depends on the previous one, which can help~\citep{abbeHowFarCan2024,houTuringPrograms2024}; the trace format in prior work and our first study require transformer to track intermediate search states across the entire trace.
